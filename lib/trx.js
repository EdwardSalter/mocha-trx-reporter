...
function ReporterTrx(runner, options) {
    ...
    runner.on('end', () => {
        ...
        if (reporterOptions.attachmentsFolderName) {
            testRunOptions.deployment = {
                runDeploymentRoot: reporterOptions.attachmentsFolderName,
            };
        }

        const run = new TestRun(testRunOptions);
