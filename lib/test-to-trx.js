...
function testToTrx(test, computerName, cwd, options) {
    ...
    if (options.attachmentsFolderName) {
        // TODO: SANITIZE FILENAME
        const fullPath = path.resolve(options.attachmentsFolderName, 'In', test.fullTitle());

        try {
            const attachments = fs.readdirSync(fullPath);
            result.relativeResultsDirectory = test.fullTitle();
            if (attachments && attachments.length > 0) {
                result.resultFiles = attachments.map(filename => ({
                    path: filename,
                }));
            }
        } catch (e) {
            // Do nothing, there are no outputs
        }
    }
